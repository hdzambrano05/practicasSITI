...
class DateTimer {
    ...
    {
        if (!is_numeric($date1)) {
            $date1 = DateTimer::timeStringToStamp($date1);
        }
        if (!is_numeric($date2)) {
            $date2 = DateTimer::timeStringToStamp($date2);
        }
        if ($date1 < $date2) {
            return -1;
        } else if ($date1 > $date2) {
    ...
    {
        if (!is_numeric($time)) {
            $time = DateTimer::timeStringToStamp($time);
        }
        ...
        return DateTimer::timeStampToString($time, $format);
    }
    ...
    function timeToHumanReadable($time = 0)
    {
        ...
        if ($time == 0) {
            ...
            if ($secs) {
                $timestring .= "$secs Segundo" . ($secs == 1 ? '' : 's');
            }
            return $timestring."@".$days."@".$hrs."@".$mins."@".$secs;
        }
    }
